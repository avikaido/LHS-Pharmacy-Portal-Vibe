...
import { SearchPharmacist } from '../../../store/apps/pharmacists/PharmacistSlice';
import { IconMenu2, IconSearch } from '@tabler/icons';

const PharmacistSearch = ({ onClick }) => {
  const searchTerm = useSelector((state) => state.pharmacistsReducer.PharmacistSearch);
  ...
  return (
    <Box display="flex" sx={{ p: 2 }}>
      ...
      <TextField
        ...
        variant="outlined"
        onChange={(e) => dispatch(SearchPharmacist(e.target.value))}
      />
    </Box>
  );
};
