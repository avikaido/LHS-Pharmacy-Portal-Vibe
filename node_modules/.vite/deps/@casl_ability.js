...
function R2(t3, i4) {
  if (i4) {
    if (!Object.hasOwn(i4, C))
      Object.defineProperty(i4, C, { value: t3 });
    else if (t3 !== i4[C])
      throw new Error(`Trying to cast object to subject type ${t3} but previously it was casted to ${i4[C]}`);
  }
  return i4;
}
var P2 = (t3) => {
...
function q3(t3, i4, e3) {
  ...
  while (n3 < s3.length) {
    const i5 = s3[n3++];
    if (Object.hasOwn(t3, i5))
      s3 = e3(s3, t3[i5]);
  }
  return s3;
}
function D(t3, i4) {
  if (typeof i4 === "string" && t3.indexOf(i4) !== -1)
    return i4;
  for (let e3 = 0; e3 < i4.length; e3++)
    if (t3.indexOf(i4[e3]) !== -1)
      return i4[e3];
  return null;
}
...
function k(t3, i4) {
  if (i4 in t3)
    throw new Error(`Cannot use "${i4}" as an alias because it's reserved action.`);
  ...
  const s3 = (t4, e4) => {
    ...
    if (n3)
      throw new Error(`Cannot make an alias to "${i4}" because this is reserved action`);
    return t4.concat(e4);
  };
...
function L(t3, i4) {
  if (!i4 || i4.skipValidate !== false)
    k(t3, i4 && i4.anyAction || "manage");
  return (i5) => q3(t3, i5, Y);
}
function U(t3, i4, e3) {
  for (let s3 = e3; s3 < i4.length; s3++)
    t3.push(i4[s3]);
}
function G(t3, i4) {
  if (!t3 || !t3.length)
  ...
  let s3 = 0;
  const n3 = [];
  while (e3 < t3.length && s3 < i4.length)
    if (t3[e3].priority < i4[s3].priority) {
      n3.push(t3[e3]);
      e3++;
    } else {
      n3.push(i4[s3]);
...
function J(t3, i4) {
  ...
  if (t3.fields && !i4.fieldMatcher)
    throw new Error('You need to pass "fieldMatcher" option in order to restrict access by fields');
  if (t3.conditions && !i4.conditionsMatcher)
...
var Z = class {
  ...
  v(t3) {
    ...
    for (let s3 = t3.length - 1; s3 >= 0; s3--) {
      ...
      for (let t4 = 0; t4 < c4.length; t4++) {
        ...
        if (e3 === void 0)
          e3 = typeof c4[t4];
        if (typeof c4[t4] !== e3 && e3 !== "mixed")
          e3 = "mixed";
        for (let t5 = 0; t5 < o3.length; t5++)
          H(s4, o3[t5], W).rules.push(r2);
      }
    }
    this.l = i4;
    if (e3 !== "mixed" && !this.M) {
  ...
  possibleRulesFor(t3, i4 = this.A) {
    ...
    if (i4 !== this.A)
      r2 = G(r2, this.possibleRulesFor(t3, this.A));
    s3.rules = r2;
    s3.merged = true;
    return r2;
  }
  ...
  actionsFor(t3) {
    ...
    if (e3)
      Array.from(e3.keys()).forEach((t4) => i4.add(t4));
    const s3 = t3 !== this.A ? this.l.get(this.A) : void 0;
    if (s3)
      Array.from(s3.keys()).forEach((t4) => i4.add(t4));
...
var at = (t3) => {
  ...
  return (e3) => {
    if (typeof i4 === "undefined")
      i4 = t3.every((t4) => t4.indexOf("*") === -1) ? null : lt(t3);
...
var AbilityBuilder = class {
  ...
  R(t3, i4, e3, s3, n3) {
    ...
    if (i4) {
      r2.subject = i4;
      if (Array.isArray(e3) || typeof e3 === "string")
        r2.fields = e3;
